...
const History = ({ onFriendClick, id, ismyprofil} ) => {
  ...
  const fetchUserFriends = async (userId) => {
    try {
      const response = await axios.get(`http://10.13.5.8:8000/api/users/${userId}/user_friends/`);
  ...
  const fetchUserBlocked = async (userId) => {
    try {
      const response = await axios.get(`http://10.13.5.8:8000/api/users/${userId}/user_blocked_friends/`);
