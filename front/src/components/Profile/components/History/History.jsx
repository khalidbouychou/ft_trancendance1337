...
const History = ({ onFriendClick, id, ismyprofil} ) => {
  ...
  const fetchUserFriends = async (userId) => {
    try {
      const response = await axios.get(`https://10.11.9.12/api/users/${userId}/user_friends/`);
  ...
  const fetchUserBlocked = async (userId) => {
    try {
      const response = await axios.get(`https://10.11.9.12/api/users/${userId}/user_blocked_friends/`);
