...
const CardMatch = ({ match, animationDelay, t}) => {
 
  let { profile_name } = useParams();
  const { user } = useContext(AuthContext)
  const navigate = useNavigate();
  const opponent = match.winner_profile_name === profile_name ? match.loser_profile_name : match.winner_profile_name;
  const status = match.winner_profile_name === profile_name ? "Victory" : "Defeat";
  const score = `${match.left_score} - ${match.right_score}`;
  const color = match.winner_profile_name === profile_name ? "linear-gradient(to right, #006437, #00b378)" : "linear-gradient(to right, #64000e, #8b0000)";
