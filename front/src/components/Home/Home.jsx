...
const Home = () => {
        ...
        const fetchSearchResults = async () => {
            if (searchQuery.trim()) {
                const response = await fetch(
                    `http://10.13.5.8:8000/api/search/?q=${searchQuery}`
                );
    ...
    const handleSearch = () => {
        if (searchQuery.trim()) {
            navigate(`/search?q=${searchQuery}`); // Assuming your search component is routed like this
        }
    };
    ...
    useEffect(() => {
        const fetchDataResults = async () => {
            try {
                const response = await fetch(`http://10.13.5.8:8000/api/pingdata/`,
                );
            ...
            <div className={styl.cont}>
                ...
                <div className={styl.first}>
                    <div className={styl.intro}>
                        ...
                        Gear up for epic battles as you challenge your friends to thrilling matches in 
                        Ping Pong Legends and Tic Tac Toe !
                        Who will rise to the top and prove their dominance on the table?
                        </p>
                    </div>
                ...
                <div className={styl.last}>
                    {isMobile ? <Tmp2 Data={pingData}/> : <Tmp1 />}
                </div>
            </div>
        </div>
    )
