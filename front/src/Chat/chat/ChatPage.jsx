...
function ChatPage() {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				const response = await axios('https://localhost/api/chat/',{
	...
	const setupNotificationSocket = () => {
		const socket = new WebSocket(`wss://localhost/ws/notification/`);
	...
	const setupSocket = (room_id) => {
		return new Promise((resolve, reject) => {
			...
			const newSocket = new WebSocket(`wss://localhost/ws/chat/${room_id}/`)
