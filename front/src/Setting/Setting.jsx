...
const Settings = () => {
  ...
  const handleProfileNameUpdate = async () => {
    try {
      const response = await axios.put(
        `http://${import.meta.env.VITE_IP_HOST}:8000/api/update/`,
  ...
  const uploadImage = () => {
    ...
    if (input.files.length > 0) {
      ...
      axios
        .put(`http://${import.meta.env.VITE_IP_HOST}:8000/api/update/`, formData, {
          withCredentials: true,
          headers: {
  ...
  const handleConfirm = async () => {
    try {
      const response = await axios.get(`http://${import.meta.env.VITE_IP_HOST}:8000/api/${actionToPerform}/` , {
        withCredentials: true,
    });
      if (response.status === 200) {
