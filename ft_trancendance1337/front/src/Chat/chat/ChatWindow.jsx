...
function ChatWindow({ currentContact, chat, message, sendMessage, handleTyping, data, chatMessagesRef, sockets, typingUser }) {
    ...
    const handlePlayPong = () => {
        ...
        if (isConnected) {
            ...
            const pong_socket = new WebSocket(`ws://10.13.6.3:8000/ws/play-friend/?token=${token}`);
            pong_socket.onopen = () => {
                const data2 = {
                    action: 'friend_game',
	...
	const handlePlayTicTacToe = () => {
		if (isConnected) {
			...
			if (!token)	return;
			try { xo_invite_socket = new WebSocket(`ws://10.13.6.3:8000/ws/xo_invite/?token=${token}`) }
			catch (err) { return }
